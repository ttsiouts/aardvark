...
from aardvark.db.sqlalchemy.nova_api_models import InstanceMapping
...
def _create_facade_lazily():
    global _FACADE
    if _FACADE is None:
        _FACADE = db_session.EngineFacade(CONF.compute.nova_api_db)
    return _FACADE
...
class NovaApiConnection(nova_api.NovaApiConnection):
    ...
    def list_preemptible_instances(self, cell_ids, preemptible_projects):
        query = model_query(InstanceMapping)
        query = query.filter_by(queued_for_delete=False)
        query = query.filter(InstanceMapping.cell_id.in_(cell_ids))
        query = query.filter(
            InstanceMapping.project_id.in_(preemptible_projects))
        try:
            return query.all()
        except NoResultFound:
            return []
